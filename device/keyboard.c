...
if (ext_scan_code){
  scan_code = (0xe000 | ext_scan_code);
  ext_scan_code = false;
}
break_code = ((scan_code & 0x80) != 0);
if (break_code){
  uint16_t make_code = (scan_code &= 0xff7f);
  if(make_code == ctrl_l_make || make_code ==\
ctrl_r_make){
    ctrl_status = false;
  }
...
make_code == alt_r_make){
    alt_status = false;
  }
  return;
}
//通码只处理数组定义的键以及alt,ctrl
